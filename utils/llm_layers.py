...
class VTILayer(nn.Module):
    ...
    def forward(self, x):
        if self.vti_direction is not None:
            ...
            if y.shape[0]* 3== x.shape[0] and len(y.shape)==2:
                y = y.repeat(3,1)
            if y.shape[0]* 3== x.shape[0] and len(y.shape)==3:
                y = y.repeat(3,1,1) 
...
def add_vti_layers(model: PreTrainedModel, vti_drections: Tensor, alpha: list, sparse_layers_list=None):
    layers = get_layers(model)
    if model.__class__.__name__ == "Qwen2_5_VLForConditionalGeneration":
        layers =  model.model.layers
    elif  model.__class__.__name__ == "Qwen2_5_VisionTransformerPretrainedModel":
        layers = model.blocks
    mlp_keywords = ["mlp", "feedforward", "ffn"]
    assert len(vti_drections) == len(layers)
    for i, layer in enumerate(layers):
        if sparse_layers_list is not None:
            if i not in sparse_layers_list:
                continue
